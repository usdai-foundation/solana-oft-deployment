use crate::{
    common::{AccountMetaRef, AddressLocator},
    COMPOSED_MESSAGE_HASH_SEED, ENDPOINT_SEED, EVENT_SEED, NONCE_SEED, OAPP_SEED,
    PAYLOAD_HASH_SEED,
};
use anchor_lang::{prelude::*, solana_program::system_program::ID as SYSTEM_ID};
use solana_keccak_hasher::hash;

/// Version identifier returned by `lz_receive_types_info` for the V2 flow.
pub const LZ_RECEIVE_TYPES_VERSION: u8 = 2;

/// Payload returned from `lz_receive_types_info` when the version is V2.
///
/// The executor uses these accounts to construct the follow-up
/// `lz_receive_types_v2` call.
